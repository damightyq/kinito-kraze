...
local bg = 'MarkLOADING' -- don't touch this
...
function onCreatePost() -- sprite loading
	makeLuaSprite('MarkLOADING', 'loadingscreen/'..bg, 0, 0)
	runTimer('fadeTimer', 6.0, 1)

	addLuaSprite('MarkLOADING', true)
	setObjectCamera('MarkLOADING', 'camOther')
end

function onTimerCompleted(tag) -- bye bye loading screen
    if tag == 'fadeTimer' then
	    doTweenAlpha('delete', 'MarkLOADING', 0, 0.5, 'backIn')
		allowCountdown = true
		startCountdown()
...
function onTweenCompleted(tag) -- everything goes bye bye
    if tag == 'delete' then
	    removeLuaSprite('MarkLOADING', true)
